...
https://infallible-thompson-49de36.netlify.app   figure3
"""

using KernelFunctions,GLMakie,Random,Distributions,LinearAlgebra
Random.seed!(123)

SE(σ)=σ^2*SqExponentialKernel()
...
function prior_sample(idx)
    ...
    axs[idx].xlabel=L"σ =%$(σs[idx])"
    axs[idx].limits=(-5,5,-3,3)
end

[prior_sample(idx) for idx in 1:3]
fig
save("./image/gp-with-different-variance.png",fig)
